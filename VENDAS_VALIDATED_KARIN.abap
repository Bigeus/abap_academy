*&---------------------------------------------------------------------*
*& Report ZTT0SIMOEV_R_VENDAS_VALIDATED
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ztt0simoev_r_vendas_validated.

TABLES: ztt0simoev_tm_v1, t001.
...
AT SELECTION-SCREEN.
  ...
  IF s_bukrs IS NOT INITIAL.
    ...
    IF sy-subrc <> 0.
      MESSAGE: |Campo não encontrado em T001| TYPE 'S' DISPLAY LIKE 'E'.
      STOP.
    ENDIF.
  ENDIF.
...
*--------------------------------------------------------------------*
  ...
  DATA: vg_tabix TYPE i.
  vg_tabix = sy-tabix.

  IF sy-subrc EQ 0.
    w_fieldcat-tech = 'X'.
    MODIFY t_fieldcat FROM w_fieldcat INDEX vg_tabix.
...
*--------------------------------------------------------------------*
* Aplicar sumarização na coluna de valor
*--------------------------------------------------------------------*
  READ TABLE t_fieldcat ASSIGNING FIELD-SYMBOL(<sum>) WITH KEY fieldname = 'VALOR'.
  IF sy-subrc EQ 0.
    <sum>-do_sum = 'X'.
    MESSAGE: 'RODOU' TYPE 'S'.
  ELSE.
    MESSAGE: 'Não aplicou o sum ao campo valor' TYPE 'S' DISPLAY LIKE 'E'.
  ENDIF.
...
*                                             OR fieldname = 'VALOR'
        ...
        <clean>-tech = 'X'.
      ENDLOOP.
*      DATA(fields_with_tech) = VALUE stringtab(
*   ( 'NRO' ) ( 'ITEM' ) ( 'MATNR' ) ( 'MAKTX' )
*   ( 'DATA_VENDA' ) ( 'KUNNR' ) ( 'UNV' ) ( 'QTD' )
* ).
*
*      LOOP AT t_fieldcat ASSIGNING <clean>
*        WHERE fieldname IN fields_with_tech.
*
*        <clean>-tech = 'X'.
*
*      ENDLOOP.
...
FORM present_alv USING t_alv TYPE ty_t_alv.
  ...
  CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
    EXPORTING
      ...
      is_layout          = wl_layout
      i_grid_title       = 'Relatório de Vendas - Karin'
    TABLES
      t_outtab           = t_alv
    EXCEPTIONS
      program_error      = 1
